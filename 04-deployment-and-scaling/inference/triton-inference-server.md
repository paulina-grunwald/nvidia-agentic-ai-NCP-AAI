...
- Triton can deploy models from multiple frameworks (**multi-Framework Support**) including:
  - TensorRT
  - PyTorch
  - ONNX
  - OpenVINO
...
> Prompt: "What is AI?"
>
> Step 1: Generate token → "AI"
> Step 2: Generate token → "is"
> Step 3: Generate token → "a"
> Step 4: Generate token → "field"
> Step 5: Generate token → "of"
> ...and so on until done
> ```
>
> Each step requires a **full forward pass through the model**.
> This is important for understanding batching.

#### Traditional Batching (Sequence-Level Batching)

- **"Sequence"** = one complete request (prompt → full response)

In sequence-level batching, you group multiple **complete requests** and process them together. The batch stays together until **ALL sequences finish**.
...
The batching decision happens **at every token generation step**:

| Aspect | Sequence-Level | Token-Level |
|--------|----------------|-------------|
| Batch formed | Once at start | Every token step |
| Sequence exits | Only when ALL done | Immediately when done |
| New sequences join | Wait for next batch | Join at any token step |
| GPU utilization | Wasted on padding | Always fully utilized |
| Latency for short requests | Poor (wait for longest) | Great (return immediately) |
...
To understand KV Cache, you need to understand how attention works in transformers.

In transformers, each token "attends" to all previous tokens using **Query (Q)**, **Key (K)**, and **Value (V)** vectors:

```
Attention(Q, K, V) = softmax(Q × K^T / √d) × V
```

For each new token, the model needs to:
...
- **Q: Why is KV cache needed in LLM inference?**
...
- **Q: How does paged attention optimize KV cache?**
  A: Allocates KV cache in non-contiguous pages, eliminating fragmentation and enabling dynamic memory management.

- **Q: What are the two phases of LLM inference?**
  A: Prefill (process prompt, compute-bound) and Decode (generate tokens, memory-bound due to KV cache access).

### Custom CUDA Kernels

A **CUDA kernel** is a function that runs on the GPU. It's the actual code that executes on thousands of GPU cores in parallel.

```
CPU code: "For each of 1 million numbers, multiply by 2"
         → Executes sequentially, one at a time

GPU kernel: "All cores: multiply your assigned number by 2"
         → Executes in parallel, thousands at once
```
...
- **Q: What is Flash Attention?**
  A: An optimized attention algorithm that computes attention in tiles, avoiding materializing the full N×N attention matrix.

### Quantization

Quantization means **reducing the precision** of numbers used to represent model weights and activations. Instead of using high-precision numbers, you use lower-precision ones.

```
Original (FP32):  3.141592653589793  (32 bits per number)
FP16:             3.140625           (16 bits per number)
INT8:             3                  (8 bits per number)
```

#### Why Does Precision Matter?

**Memory:** Each parameter takes space

```
Llama 70B model:
- FP32: 70 billion × 4 bytes = 280 GB  (won't fit on any single GPU!)
- FP16: 70 billion × 2 bytes = 140 GB  (still huge)
- INT8: 70 billion × 1 byte  = 70 GB   (fits on large GPUs)
- INT4: 70 billion × 0.5 bytes = 35 GB (fits on consumer GPUs!)
```

**Speed:** Lower precision = faster math
- GPU can do more INT8 operations per second than FP32 operations
- → Same model runs faster with lower precision

#### Exam Points on Quantization

- **Q: How do you reduce LLM memory footprint?**
  A: Use quantization (INT8, INT4) to reduce precision and memory usage.

- **Q: What's the tradeoff with quantization?**
  A: Lower precision = less memory + faster inference, but slightly reduced accuracy.

- **Q: Which precision for production LLM inference?**
  A: *FP16* is default, *INT8* for better efficiency with minimal accuracy loss.

### Tensor Parallelism

**The Problem:** Large LLMs don't fit on a single GPU.

**Solution:** Split the model across multiple GPUs.

Tensor Parallelism **splits individual layers horizontally** across GPUs. Each GPU holds a **slice** of every layer.

#### Exam Quick Points

| Question | Answer |
|----------|--------|
| **What is Tensor Parallelism?** | Splitting model layers across GPUs horizontally |
| **Why use it?** | Run models larger than single GPU memory |
| **Tradeoff?** | Requires fast GPU-to-GPU communication |
| **Best interconnect?** | NVLink (high bandwidth, low latency) |
| **TensorRT-LLM support?** | Yes, configurable with `tensor_parallel_size` |
...
> When you see questions about **Triton Inference Server** + **performance optimization** + **real-time**, dynamic batching is almost always the answer.
...
- Model file name depends on backend (e.g., `model.plan` for TensorRT, `model.pt` for PyTorch)
